...
export abstract class IRnboService<Target, Data, Meta> {
  ...
  setVal(device_id: string | null, key: string | null, value: Data|null): void {
    let subject =  this.getObj(device_id, key)?.subject;
  ...
  bindToControl(device_id: string|null, key: string|null, control: FormControl<Data | null>): Subscription|null { 
    let obj = this.getObj(device_id, key);
    if(!obj){
        return null;
    }
    this.subscribe(obj, (value: Data|null) => value ? control.setValue(value) : null);
    return control.valueChanges.subscribe((value: Data|null) => this.setVal(device_id, key, value));
